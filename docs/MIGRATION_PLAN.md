...
**Deliberately left blank rather than invented**: `OurVision` (no sourced vision statement distinct from the mission text), the `leadership` table and `PrincipalName`/`MinisterName` site_settings keys (no current principal's name found in any public source — the About/Home pages just don't render those sections when unset, which is the correct behavior for genuinely unknown data), `portal_news` (nothing current — the only "news" found was a stale 2014 exam date from an outdated third-party site), `office_hours` (no verified hours for this specific college), `program_fees`/`additional_fee_*` (a fee-structure lookup site explicitly stated fees weren't listed), `downloads`, `faculty_directory`/`faculty_categories` (no named faculty found), `apply_steps`/`important_dates`/`department_contacts`. If you have the real values for any of these — especially the principal's name — they're a straightforward follow-up.
