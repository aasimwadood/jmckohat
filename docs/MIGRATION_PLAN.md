...
**Not yet done**: the four migrations haven't been applied to any Postgres instance (no Supabase CLI/Docker/psql available in this environment — same constraint noted throughout Phase 15/§8), so nothing here has been RLS/RPC-verified live the way every other phase in this document was. Also unverified: an actual browser walkthrough of the full pipeline (advertise → apply → scrutinize → shortlist → interview → select → issue order). Both are the concrete next step before this ships — `npx supabase db push` against the real project, then the same kind of live-account RLS verification §8/§9 describe, plus one real click-through. The project-wide Storage-upload infrastructure bug (§8, still unresolved as of this phase) will block real applicant document uploads specifically until it's fixed on the Supabase project side — this is an existing, known blocker, not something new introduced here.
