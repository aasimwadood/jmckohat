...
- **Promotions**: ~~`fee_pending` (in the status enum) is never set by any function — appears to be an intentional collapsed state machine per the code's own comment, not an accidental gap, but the enum value is dead weight.~~ **Resolved** — Fee Management Phase 1 (migration `0062`) repurposed `fee_pending` as the new first state of the promotion lifecycle, ahead of `pending_registration`, so fee clearance now genuinely gates course registration instead of following it. See MIGRATION_PLAN.md §35. `cgpa`/`academic_standing` are still never written by any code path despite a comment claiming `max_courses` derives from academic standing — that derivation still doesn't exist; `max_courses` is still just a fixed default.
...
- **Audit log coverage is real but inconsistent** — present for admissions, staff/org provisioning, recruitment, and promotion *fee verification only* (now also fee-structure changes, voucher generation, bank-import upload/confirm, and manual fee overrides, per Fee Management Phase 1 — MIGRATION_PLAN.md §35); still missing from FYP evaluation finalization, results submission, promotion cycle start/course registration, and role-permission changes.
