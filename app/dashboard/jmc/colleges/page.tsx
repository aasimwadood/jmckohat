...
export default async function JmcCollegesPage() {
  ...
  const [{ data: colleges }, { data: collegeTypes }, { data: admins }] = await Promise.all([
    supabase.from("colleges").select("*").eq("jmc_id", profile.jmcId).order("name"),
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle>Colleges in Your JMC</CardTitle>
          <CreateCollegeDialog jmcs={[]} collegeTypes={collegeTypes ?? []} fixedJmcId={profile.jmcId} />
        </div>
      </CardHeader>
      <CardContent>
        <Table>
