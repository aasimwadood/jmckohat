...
export default async function ProgramsPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [{ data: categories }, { data: programs }] = await Promise.all([
    ...
    supabase.from("program_details").select("*").eq("college_id", college.id).order("display_order"),
  ]);
                      ...
                      <Card key={program.id} className="transition-all duration-300 hover:shadow-xl">
                        <CardContent className="p-6">
                          ...
                          <div className="flex gap-2">
                            <Link href={`${basePath}/how-to-apply`} className="flex-1">
                              <Button className="w-full">Apply Now</Button>
                            </Link>
                            <Link href={`${basePath}/requirements`}>
