...
export default async function HowToApplyPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [{ data: steps }, { data: importantDates }] = await Promise.all([
    supabase.from("apply_steps").select("*").eq("college_id", college.id).order("step_number"),
    supabase.from("important_dates").select("*").eq("college_id", college.id).order("display_order"),
  ]);
      ...
      <section className="bg-gradient-to-r from-blue-600 to-blue-800 py-16 text-white">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <p className="max-w-3xl text-xl text-blue-100">
            Follow our step-by-step application process to join {college.name}.
          </p>
        </div>
      </section>
            ...
            <CardContent className="p-8">
              <div className="flex flex-col items-center justify-between gap-6 md:flex-row">
                ...
                <div className="flex flex-wrap gap-4">
                  ...
                  <Link href={`${basePath}/fee-structure`}>
