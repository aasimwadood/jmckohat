...
export default async function AboutPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [{ data: settings }, { data: leaders }, { data: departments }, { data: programs }, { data: highlights }] =
    await Promise.all([
      ...
      supabase.from("leadership").select("*").eq("college_id", college.id).order("display_order"),
      supabase.from("departments").select("id, name, description").eq("college_id", college.id).order("name"),
      supabase.from("programs").select("department_id, name"),
      supabase.from("portal_stats").select("*").eq("college_id", college.id).order("display_order"),
    ]);
    ...
    <div className="flex min-h-screen flex-col bg-gray-50">
      <section className="bg-gradient-to-r from-blue-600 to-blue-800 py-16 text-white">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <p className="max-w-3xl text-xl text-blue-100">{configMap["AboutUs"] ?? college.aboutContent}</p>
        </div>
      </section>
