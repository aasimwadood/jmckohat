...
export default async function ContactPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [{ data: contacts }, { data: officeHours }, { data: deptContacts }, { data: departments }, { data: campuses }] =
    await Promise.all([
      supabase.from("contact_info").select("*").eq("college_id", college.id).order("display_order"),
      supabase.from("office_hours").select("*").eq("college_id", college.id).order("display_order"),
      supabase.from("department_contacts").select("id, department_id, phone, email").eq("college_id", college.id),
      supabase.from("departments").select("id, name").eq("college_id", college.id),
      supabase.from("campus_locations").select("*").eq("college_id", college.id),
    ]);
