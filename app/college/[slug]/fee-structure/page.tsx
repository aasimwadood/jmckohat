...
export default async function FeeStructurePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [{ data: categories }, { data: programFees }, { data: feeCategories }, { data: feeItems }] =
    await Promise.all([
      ...
      supabase.from("program_fees").select("*").eq("college_id", college.id).order("display_order"),
      supabase.from("additional_fee_categories").select("*").eq("college_id", college.id).order("display_order"),
      supabase.from("additional_fee_items").select("*").eq("college_id", college.id).order("display_order"),
    ]);
