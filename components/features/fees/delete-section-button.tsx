...
import { deleteSectionAction } from "@/lib/actions/groups-sections";

export function DeleteSectionButton({ id }: { id: string }) {
  const [isPending, startTransition] = useTransition();

  const onClick = () => {
    if (!confirm("Remove this section? Students already assigned to it must be reassigned first.")) return;
